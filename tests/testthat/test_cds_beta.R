...
server_check <- ecmwfr:::ecmwf_running(ecmwfr:::wf_server(service = "cds_beta"))
...
test_that("cdsBeta datasets returns data.frame or list", {
  skip_on_cran()
  skip_if(login_check)
...
test_that("cds_beta request", {
  ...
  expect_error(
    wf_request(
      user = "2088",
      request = cds_beta_request_faulty
    )
  )
...
test_that("required arguments missing for cds_Beta_* functions", {
  skip_on_cran()
  skip_if(login_check)
  ...
  expect_output(str(wf_datasets(user = "2088", service = "cds_beta")))
